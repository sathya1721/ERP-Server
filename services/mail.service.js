...
exports.sendMailFromStoreWithAttachment = function(jsonData, callback) {
    ...
    transporter.sendMail(mailOptions, (err, info) => {
        if(!err && info) {
            if(fs.existsSync(fileUrl)) { fs.unlink(fileUrl, function (err) {}); }
            callback(null, info);
        }
        else { callback(err, 'Mail send failed'); }
    });
                        ...
                        request.get(filePath, function (err, response, body) {
                            ...
                            if(!customerEmail) {
                                if(storeDetails.device_token) {
                                    ...
                                    notifyService.push({ subscribers: allSubscriptions, payload: notificationPayload });
                                }
    ...
    return new Promise((resolve, reject) => {
        orderList.aggregate([
            ...
            if(!err && response[0]) {
                let orderDetails = response[0];
                let storeDetails = orderDetails.storeDetails[0];
                let vendorList = orderDetails.storeFeatures[0].vendors.filter(obj => obj.status=='active');
                let mailConfig = setupConfig.mail_config;
                if(storeDetails.mail_config.transporter) { mailConfig = storeDetails.mail_config; }
                if(vendorList.length && orderDetails.vendor_list && orderDetails.vendor_list.length) {
                        ...
                        request.get(filePath, function (err, response, body) {
                            ...
                            for(let vendor of orderDetails.vendor_list) {
                                ...
                                if(vendorIndex!=-1) {
                                    ...
                                    mailContent = mailContent.replace("##vendor_name##", vendorDetails.name);
    ...
    return new Promise((resolve, reject) => {
        orderList.aggregate([
            ...
            if(!err && response[0]) {
                ...
                let orderVendorDetails = orderDetails.vendor_list.filter(obj => String(obj.vendor_id)==String(vendorDetails._id));
                if(storeDetails.mail_config.transporter) {
                    ...
                    let currentDate = dateFormat(new Date(orderVendorDetails[0].confirmed_on), "mmmm d yyyy")+' at '+dateFormat(new Date(orderVendorDetails[0].confirmed_on), "h:MM:ss TT");
                    mailTemp.vendor_order_confirmed(storeDetails).then((body) => {
                        ...
                        request.get(filePath, function (err, response, body) {
                            if(!err && response.statusCode == 200) { bodyContent = body; }
                            bodyContent = bodyContent.replace("##vendor_name##", vendorDetails.name);
                            bodyContent = bodyContent.replace("##order_number##", orderDetails.order_number);
                            ...
                            let sendData = {
                                ...
                                subject: vendorDetails.name+" has confirmed the order with order ID: "+orderDetails.order_number+".",
                                body: bodyContent
                            };
                            // send mail
                            _this.sendMailFromStore(sendData, function(err, response) {
            ...
            if(!err && response[0]) {
                ...
                if(vendorList.length && orderDetails.vendor_list && orderDetails.vendor_list.length) {
                    ...
                    mailTemp.order_cancelled_vendor(storeDetails).then((body) => {
                        let bodyContent = body;
                        let filePath = setupConfig.mail_base+storeDetails._id+'/order_cancelled_vendor.html';
                        request.get(filePath, function (err, response, body) {
                            if(!err && response.statusCode == 200) { bodyContent = body; }
                            for(let vendor of orderDetails.vendor_list) {
                                let vendorIndex = vendorList.findIndex(obj => String(obj._id)==String(vendor.vendor_id));
                                if(vendorIndex!=-1) {
                                    ...
                                    let vendorDetails = vendorList[vendorIndex];
                                    mailContent = mailContent.replace("##vendor_name##", vendorDetails.name);
                                    mailContent = mailContent.replace("##order_number##", orderDetails.order_number);
                                    mailContent = mailContent.replace("##order_number##", orderDetails.order_number);
                                    mailContent = mailContent.replace("##order_date##", orderCreatedDate);
                                    mailContent = mailContent.replace("##copy_year##", new Date().getFullYear());
                                    let sendData = {
                                        store_name: storeDetails.name,
                                        config: mailConfig,
                                        sendTo: vendorDetails.email,
                                        subject: "Your order with order ID: "+orderDetails.order_number+" from "+storeDetails.name+" has been cancelled",
                                        body: mailContent
                                    };
                                    // send mail
                                    _this.sendMailFromStore(sendData, function(err, response) {
