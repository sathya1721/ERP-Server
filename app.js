...
app.get('/', function(req, res) { res.send({ message : 'Welcome to ERP version 0.1' }); });
...
app.post('/logo_upload', function(req, res) {
	...
	if(!fs.existsSync(req.body.root_path)) {
		fs.mkdir(req.body.root_path, { recursive: true }, (err) => {
			if(!err) {
				imgUploadService.logoUploadFromRequest(req.body).then((respData) => {
	            ...
	            if(!err && response) {
					...
					// store
						...
						if(response.login_type=='admin') {
							...
							storeFeatures.findOne({ store_id: mongoose.Types.ObjectId(response.id), "vendors._id": mongoose.Types.ObjectId(response.vendor_id),
							"vendors.session_key": response.session_key, "vendors.status": "active" }, function(err, response) {
								if(!err && response) {
									next();
								}
								else {
